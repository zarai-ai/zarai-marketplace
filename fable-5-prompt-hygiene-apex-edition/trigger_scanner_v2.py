#!/usr/bin/env python3
"""
Fable 5 Trigger Scanner v2 — Updated for research-term whitelisting and XML structure detection.

This is an ENHANCED version of the original scanner with the following improvements:
1. Duplicate "entropy" key in CONTEXT_DEPENDENT fixed (kept the detailed version)
2. RESEARCH_TERMS whitelist added — proper nouns like "Text-Grad" that contain trigger
   substrings but are legitimate academic terms are now flagged but NOT counted as triggers
3. XML heavy structure detection — prompts with 4+ XML-style tags get a warning
4. MEDIUM risk now includes XML structural warnings (previously only counted triggers)

Usage: python3 trigger_scanner_v2.py <prompt_text_file>
       cat prompt.txt | python3 trigger_scanner_v2.py -
       python3 trigger_scanner_v2.py --interactive
"""
...
# Research terms and proper nouns that contain trigger substrings but are
# legitimate academic/technical terms. These are whitelisted — if found in
# the text, the scanner notes them but does NOT count the embedded trigger.
RESEARCH_TERMS = {
    "text-grad": "gradient",
    "evo prompt": "evolutionary",
    "evo-prompt": "evolutionary",
    "chain-of-thought": "chain-of-thought",
    "chain of thought": "chain-of-thought",
    "auto-regressive": "regressive",
    "top-k": "k",
    "top-p": "p",
    "clip model": "clip",
    "dreambooth": "booth",
    "proteinformer": "protein",
    "genetic algorithm": "genetic",
    "genetic programming": "genetic",
    "differential evolution": "evolution",
    "evolution strategy": "evolution",
    "evolutionary algorithm": "evolutionary",
    "evolutionary computation": "evolutionary",
    "neural architecture search": "neural",
    "neural tangent kernel": "neural",
}
...
def scan_triggers(text: str) -> Dict:
    ...
    results = {
        ...
        "research_term_warnings": [],
        "xml_structure_warning": None,
        "overall_risk": "low"
    }

    # --- Phase 0: Detect whitelisted research terms ---
    whitelisted_triggers = set()
    for compound, embedded in RESEARCH_TERMS.items():
        if compound in text_lower:
            whitelisted_triggers.add(embedded)
            results["research_term_warnings"].append({
                "term": compound,
                "embedded_trigger": embedded,
                "note": "Whitelisted research/proper noun — trigger NOT counted"
            })
    ...
    for category, levels in TRIGGER_DB.items():
        ...
        for severity, words in levels.items():
            for word in words:
                # Skip if this word is whitelisted via a research term
                if word in whitelisted_triggers:
                    continue
    ...
    # XML heavy structure check
    xml_tags = re.findall(r"<[a-z_]+>", text_lower)
    if len(xml_tags) >= 4:
        results["xml_structure_warning"] = {
            "tag_count": len(xml_tags),
            "sample_tags": xml_tags[:6],
            "note": "Heavy XML-style tag usage detected — may trigger structural pattern matching"
        }
    ...
    has_xml_warning = 1 if results["xml_structure_warning"] else 0
    ...
    elif total_triggers >= 1 or structural_count >= 1 or has_xml_warning:
...
def format_report(results: Dict) -> str:
    ...
    # Research term warnings (whitelisted)
    if results.get("research_term_warnings"):
        lines.append("-" * 40)
        lines.append("RESEARCH TERMS WHITELISTED")
        lines.append("-" * 40)
        for w in results["research_term_warnings"]:
            lines.append(f"  '{w['term']}' (embedded: '{w['embedded_trigger']}') — {w['note']}")
        lines.append("")
    ...
    # XML structure warning
    if results.get("xml_structure_warning"):
        lines.append("-" * 40)
        lines.append("XML STRUCTURE WARNING")
        lines.append("-" * 40)
        w = results["xml_structure_warning"]
        lines.append(f"  Detected {w['tag_count']} XML-style tags")
        lines.append(f"  Sample: {', '.join(w['sample_tags'])}")
    ...
    if results["overall_risk"] == "CRITICAL":
        ...
        lines.append("  ○ Low-level triggers or structural concerns present")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 trigger_scanner_v2.py <file> OR cat file | python3 trigger_scanner_v2.py -")
        sys.exit(1)
